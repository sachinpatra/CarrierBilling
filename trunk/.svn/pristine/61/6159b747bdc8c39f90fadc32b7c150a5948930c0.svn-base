...
@interface VoiceMailViewController () <ChatGridCellDelegate, UISearchBarDelegate, UIScrollViewDelegate, UITableViewDataSource, UITableViewDelegate, SettingProtocol, ContactTableViewCellDelegate,NSFetchedResultsControllerDelegate,UIActionSheetDelegate,UITextViewDelegate>
{
        ...
        if ([self isReachMeActive]) {
            ...
            if ((_currentFilteredList && [_currentFilteredList count] > 0) || (numberOfRows > 0)) {
                ...
                if([cc.voicemailList count])
                    ...
                    self.msgLabel.text = @"Once you get a Voicemail, you'll see it listed here.";
            }
        }else{
            self.enableVoiceMailSettingsView.hidden = NO;
            self.headerLabel.text = @"Free Visual Voicemails";
            self.detailsLabel.text = @"Activate InstaVoice ReachMe and allow your callers to leave a voicemail, when you are busy or your phone is unreachable.";
...
-(int)handleEvent:(NSMutableDictionary *)resultDic
{
    if(resultDic != nil)
    {
        ...
        switch (evType)
        {
            ...
            {
                EnLogd(@"VoiceMailViewController - GET_VOICEMAIL_LIST");
                KLog(@"VoiceMailViewController - GET_VOICEMAIL_LIST");
...
-(void)dismissedChatMobileNumberViewController:(id)sender
{
    ...
    {
        [appDelegate.dataMgt setCurrentChatUser:newDic];
        //DEC 28, 2017 [appDelegate.stateMachineObj setNavigationController:self.navigationController];
...
-(void)pausePlayingAction
{
    ...
    [self stopAudioPlayback];
    //_buttonTag = [NSIndexPath indexPathForRow:-1 inSection:0];
}
