...
class SelectCountryViewController: UITableViewController {
    ...
    func fetchPlanlist() {
        ...
        purchaseAPI?.callNetworkRequest(reqDisc, withSuccess: {[weak self] (requestAPI, responseDisc) in
            ...
            guard let subList = responseDisc?.value(forKey: "sub_list") as? [[String:Any]], subList.count > 0 else {
                let alert = UIAlertController(style: .alert, message: "Unfortunately, subscription plans are not available at this moment. Please try again later")
