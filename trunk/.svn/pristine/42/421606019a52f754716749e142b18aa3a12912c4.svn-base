...
+ (void)enableLogs:(OrtpLogLevel)level {
	...
	if (level == 0) {
		...
		//CMP NSLog(@"I/%s/Disabling all logs", ORTP_LOG_DOMAIN);
	} else {
		//CMP NSLog(@"I/%s/Enabling %s logs", ORTP_LOG_DOMAIN, (enabled ? "all" : "application only"));
	...
	if ([formatedString containsString:@"\n"]) {
		...
		for (int i = 0; i < myWords.count; i++) {
			...
			if (((NSString *)myWords[i]).length > 0) {
				//CMP NSLog(@"[%@] %@%@", lvl, tab, (NSString *)myWords[i]);
			}
		}
	} else {
		//CMP NSLog(@"[%@] %@", lvl, [formatedString stringByReplacingOccurrencesOfString:@"\r\n" withString:@"\n"]);
	}
}
