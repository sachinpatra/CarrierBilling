...
public protocol CountdownTimerDelegate: NSObjectProtocol {
    /**
...
public class CountdownTimer: UIView {
    ...
    public func start() {
        ...
        CountdownTimer.validateInputTime(self.totalTime)
        timer = Timer.scheduledTimer(timeInterval:REFRESH_INTERVAL,
                                     target: self,
                                     selector:#selector(CountdownTimer.timerFired),
                                     userInfo: nil,
                                     repeats: true);
    ...
    private func addBaseSubviews() {
        ...
        self.addConstraint(NSLayoutConstraint(item: label, attribute: NSLayoutAttribute.centerX, relatedBy: NSLayoutRelation.equal, toItem: self, attribute: NSLayoutAttribute.centerX, multiplier: 1, constant: -6))
        timerLabel = label
    }
    ...
    private func updateTimerLabel(timeInterval : TimeInterval) {
        ...
        let string = String.init(format:"%01d:%02d", minutes, seconds)
        timerLabel?.text = """
                              \(string)
                              Left
                           """
