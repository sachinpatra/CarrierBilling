...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withText:(NSString *)text
{
    ...
    //DC MAY 26 2016
    NSAttributedString *rectAttributedString ;
    if (text.length) {
        rectAttributedString = [[NSAttributedString alloc]initWithString:text  attributes:@{NSFontAttributeName:font}];
    }
    else
        rectAttributedString = [[NSAttributedString alloc]initWithString:@""  attributes:@{}];
    
    CGRect rectTextStringRect = [rectAttributedString boundingRectWithSize:CGSizeMake(screenSize.width - kHorizontalMargin*4.f, 1000.f) options:NSStringDrawingUsesLineFragmentOrigin context:nil];
    
    CGSize textSize = rectTextStringRect.size;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withText:(NSString *)text
{
    ...
    //DC MAY 26 2016
    NSAttributedString *rectAttributedString ;
    if (rectAttributedString.length) {
        rectAttributedString = [[NSAttributedString alloc]initWithString:text  attributes:@{NSFontAttributeName:font}];
    }
    else
        rectAttributedString = [[NSAttributedString alloc]initWithString:@""  attributes:@{}];
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withViewArray:(NSArray *)viewArray
{
    ...
    //DC MAY 26 2016
    NSAttributedString *textSizeAttributedString ;
    if (title.length) {
        textSizeAttributedString = [[NSAttributedString alloc]initWithString:title  attributes:@{NSFontAttributeName:kTitleFont}];
    }
    else
        textSizeAttributedString = [[NSAttributedString alloc]initWithString:@""  attributes:@{}];
    
    CGSize titleSize = textSizeAttributedString.size;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withStringArray:(NSArray *)stringArray
{
    ...
    for (NSString *string in stringArray) {
        ...
        NSAttributedString *textSizeAttributedString ;
        if (string.length) {
            textSizeAttributedString = [[NSAttributedString alloc]initWithString:string  attributes:@{NSFontAttributeName:font}];
        }
        else
            textSizeAttributedString = [[NSAttributedString alloc]initWithString:@""  attributes:@{}];
        CGSize textSize = textSizeAttributedString.size;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withContentView:(UIView *)cView {
    ...
    [UIView animateWithDuration:0.0f delay:0.f options:UIViewAnimationOptionCurveEaseInOut animations:^{
        self.alpha = 1.f;
...
- (void)didTapButton:(UIButton *)sender
{
    ...
    if (index == NSNotFound) { //TODO: CHECK
        return;
    }
