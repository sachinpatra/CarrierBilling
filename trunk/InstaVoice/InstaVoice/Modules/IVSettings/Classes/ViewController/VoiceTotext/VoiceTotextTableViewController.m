...
//  InstaVoice
//
//  Created by Bhaskar Munireddy on 15/03/18.
...
- (void)viewDidLoad {
    ...
    self.detailLabel.text = @"Audio message will be converted into text by the voice recognition software.\n\n\nOn enabling, you can request a transcription for an individual Voicemail or Voice Message by tapping on Voice-To-Text button on Audio player.";
    
    NSMutableAttributedString *transcriptionCharges = [[NSMutableAttributedString alloc] initWithString:@"Charges: You need sufficient InstaVoice credits to avail this feature. Find more details about country specific pricing and FAQs at www.instavoice.com/faqs"];
...
- (IBAction)voiceToTextSwitch:(id)sender {
    ...
    if (self.transcriptionSwitch.isOn) {
        ...
        UIAlertController *turnOnAlert = [UIAlertController alertControllerWithTitle:@"Turn on Voice-To-Text" message:@"Turning on Voice-To-Text will enable a button for Voicemail transcription. When you tap on transcribe button, transcription will happen at the background and it may take longer. Transcription confidence will be indicated low, if audio is difficult to recognise. An error will be shown, if confident audio transcription is not available." preferredStyle:UIAlertControllerStyleAlert];
