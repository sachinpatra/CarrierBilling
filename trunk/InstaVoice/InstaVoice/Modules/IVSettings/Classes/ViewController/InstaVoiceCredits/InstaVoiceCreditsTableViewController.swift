//
//  InstaVoiceCreditsTableViewController.swift
//  InstaVoice
//
//  Created by Bhaskar Munireddy on 10/04/18.
...
class InstaVoiceCreditsTableViewController: UITableViewController {
    ...
    @IBAction func onInfoButtonClicked(_ sender: UIBarButtonItem) {
        let alert = UIAlertController(style: .alert, title: "Do more with InstaVoice Credits",
                                      message:"""
                                                \u{2022} Read your Voicemails by converting them in the text.
                                                \u{2022} Send local / international voice messages, SMS and photos to anyone, even if they are not using InstaVoice.
                                              """.localized)
    ...
    func purchaseCredit() {
        ...
        availableProdcutAPI?.callNetworkRequest(toFetchProductList: reqDisc, withSuccess: {[weak self] (requestAPI, responseDisc) in
            ...
            if let productList = responseDisc!["product_list"] as? [[String:Any]], let ivProduct = productList.first { //First product always use for IVCredits
    ...
    func setupInAppPurchase() {
        SwiftyStoreKit.completeTransactions(atomically: false) { purchases in
           /* ANLoader.showLoading("", disableUI: true)
            ...
            let dispatchQueue = DispatchQueue(label: "pendingTransactionsQueue")
            let dispatchSemaphore = DispatchSemaphore(value: 0)

            dispatchQueue.async {

                for purchase in purchases {//The transactions which are not finshed will be list here, finish it after local server success
                    ...
                    case .purchased:
                        ...
                        availableProdcutAPI?.callNetworkRequest(toFetchProductList: reqDisc, withSuccess: {[weak self] (requestAPI, responseDisc) in
                            ...
                            if let productList = responseDisc!["product_list"] as? [[String:Any]], let ivProduct = productList.first { //First product always use for IVCredits
                                ...
                                SwiftyStoreKit.fetchReceipt(forceRefresh: true, completion: { receiptResult in
                                    switch receiptResult {
                                    case .success(let receiptdata):
                                        ...
                                        if let receiptJSONData = try? JSONSerialization.data(withJSONObject: responseDisc, options: .prettyPrinted) {
                                            ...
                                            purchaseAPI?.callNetworkRequest(purchaseProductReqDic, withSuccess: {[weak self] (requestAPI, purchaseResponseDisc) in
                                                guard purchaseResponseDisc?.value(forKey: STATUS) as! String == STATUS_OK else {
                                                    print("Error in pending purchase product response. api request: \(String(describing: requestAPI?.request))")
                                                    return
                                                }
...
//                                                    }
                                                    dispatchSemaphore.signal()
                                                    dispatchGroup.leave()
                                                    print("Error in pending Purchase Prodcut Command: \(error.debugDescription)")
                                                    return
                                            })
                                        }
                                    case .error(let error):
...
extension InstaVoiceCreditsTableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = "Instavoice Credits"
