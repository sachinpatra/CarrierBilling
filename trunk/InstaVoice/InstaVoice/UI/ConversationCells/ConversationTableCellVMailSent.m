//
//  ConversationTableCellVMailSent.m
//  InstaVoice
//
...
#import "ConversationTableCellVMailSent.h"
#import "IVColors.h"

@interface ConversationTableCellVMailSent ()
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *tickMarkConstraints;
@end

@implementation ConversationTableCellVMailSent
...
-(void)configureCell
{
    ...
    NSString *fromNumner = [self.dic valueForKey:NATIVE_CONTACT_ID];
    NSString *theFromPhoneNumber = [Common getFormattedNumber:fromNumner withCountryIsdCode:nil withGivenNumberisCannonical:YES];
    NSString *toNumnber = [self.dic valueForKey:@"FROM_USER_ID"];
    NSString *theToPhoneNumber = [Common getFormattedNumber:toNumnber withCountryIsdCode:nil withGivenNumberisCannonical:YES];
    
    if(!theToPhoneNumber) {
        if(toNumnber.length)
            theToPhoneNumber = toNumnber;
        else
            theToPhoneNumber = @"";
    }
    
    if(!theFromPhoneNumber) {
        if(fromNumner.length)
            theFromPhoneNumber = fromNumner;
        else
            theFromPhoneNumber = @"";
    }
