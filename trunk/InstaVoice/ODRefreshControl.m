...
- (id)initInScrollView:(UIScrollView *)scrollView activityIndicatorView:(UIView *)activity
{
    ...
    if (self) {
        ...
        _activity.center = CGPointMake(floor(self.frame.size.width / 4), floor(self.frame.size.height / 2));
        _activity.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
        _loadOldDataLabel = [[UILabel alloc] init/*WithFrame:CGRectMake(floor(self.frame.size.width / 2 - 60), floor(self.frame.size.height - 37), 170,30 )*/];
        _loadOldDataLabel.frame = CGRectMake(28, -5, 200, 30);
        _loadOldDataLabel.backgroundColor = [UIColor clearColor];
        //DC MAY17 2016
        _loadOldDataLabel.textColor = [IVColors colorWithHexString:@"666666"];//[UIColor grayColor];
        [_loadOldDataLabel setFont:[Common preferredFontForTextStyleInApp:UIFontTextStyleFootnote]];
         //[UIFont fontWithName:HELVETICANEUE_MEDIUM size:SIZE_13]];
        _loadOldDataLabel.textAlignment = NSTextAlignmentLeft;
         _loadOldDataLabel.text = @"Loading older messages";
        label.hidden = NO;
        _activity.alpha = 0;
        ...
        //DC MAY17 2016
        [label setFont:(__bridge CFTypeRef _Nullable)(_loadOldDataLabel.font)];//HELVETICANEUE_MEDIUM];
        [label setFontSize:_loadOldDataLabel.font.pointSize];//SIZE_13];
        //[label setString:@"Pull to load older messages"];
        //Above commented by vinoth for removing pull to refresh keyboard swipe down
        [label setString:@""];
        [label setAlignmentMode:kCAAlignmentLeft];
        //DC MAY 17 2016
        [label setForegroundColor:[[IVColors colorWithHexString:@"666666"]CGColor]];
        ...
        [_activity addSubview:_loadOldDataLabel];
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if (_refreshing) {
        if (offset != 0) {
            ...
            _activity.center = CGPointMake(floor(self.frame.size.width / 4), MIN(offset + self.frame.size.height + floor(kOpenedViewHeight / 2), self.frame.size.height - kOpenedViewHeight/ 2));
    ...
    CGPoint bottomOrigin = CGPointMake(floor(self.bounds.size.width / 4), self.bounds.size.height - currentBottomPadding -currentBottomRadius);
    CGPoint topOrigin = CGPointZero;
    if (distance == 0) {
        topOrigin = CGPointMake(floor(self.bounds.size.width / 4), bottomOrigin.y);
    } else {
        topOrigin = CGPointMake(floor(self.bounds.size.width / 4), self.bounds.size.height + offset + currentTopPadding + currentTopRadius);
        if (percentage == 0) {
    ...
    if (!triggered) {
        ...
        //DC MAY17 2016
        label.frame = CGRectMake(topOrigin.x+20,topOrigin.y - arrowBigRadius , 210, 30);
        ...
        //Below line added by vinoth for removing pull to refresh keyboard swipe down
        [label setString:@"Pull to load older messages"];
    } else {
